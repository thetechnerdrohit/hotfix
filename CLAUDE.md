...
- **v2.4 — ONLINE-ONLY + random characters (2026-07-13):** single-player ripped out (`main.js` ONLINE always true; `match===null`; Play always joins Colyseus; difficulty menu removed). Each fighter gets a server-assigned 16-bit `skin` seed (synced, 2 bytes); `src/net/skins.js` `deriveAppearance(skin,team)` → randomized kour character (hooded tracksuit Squid DNA, team-tinted outfit, ○△□ badge, random hat) built by `avatars.js`.
- **v2.5 — Havana is the ONLINE map + positional audio (2026-07-13):** `server/sim/worldLoader.js` loads Havana (client default too; DEV `?room=battle` keeps the old field). `havanaMap.js` set-dressed kour-style (fences, containers, wires, barrels, dead trees). Full 3D-panned combat audio (shots/death/kill by world pos; self-kill centered vs others positional). Fixed a latent Phase-5 bug: client `_decodeEvents` was a passthrough → online kill-feed names were undefined.
- **v2.6 — weapon realism (2026-07-13):** rifle/pistol/knife viewmodels detailed up one step (receiver/optic/mag, slide/frame, real blade); cosmetic, muzzle anchors frozen.
- **v2.7 — TDM + FREE-FOR-ALL + matchmaking + 10-min + result screen (2026-07-13):** two Play buttons → `joinOrCreate('tdm'|'ffa')` (both registered in `server/index.js`). **TDM** 5+5 bots (replace/backfill). **FFA** humans-only, no bots, empty room grows per join, team `'ffa'` (everyone-vs-everyone), per-player `frags`, cap `MATCH.ffaMaxPlayers`. **10-min matches both** (`timeLimit` 600, `killTarget` 0). RESULT SCREEN now fires online off phase→'over' (TDM winner+score / FFA leaderboard). FFA HUD = frags board + live leaderboard. Fixed a live-only crash: `_colliderPool` now grows as FFA slots are added. **Serve two rooms: `npm run server` exposes tdm + ffa.**
