...
- Dev URL params (dev builds only): `?room=test` (feel gym + verticality fixture) · `?room=prod` (the old Prod arena) · `?bots=0` (dummy practice) · `?nolock=1` (headless testing, exposes `window.__game`) · `?online=1` (join the Colyseus server).
